table 50007 "ODT LSD Directions"
{
  ...
  var tmpBlob: Record TempBlob temporary;
  begin
    CLEAR(Directions);
    ...
    tmpBlob.Blob:=Directions;
    tmpBlob.WriteAsText(NewDirection, TextEncoding::Windows);
    Directions:=tmpBlob.Blob;
    MODIFY;
  end;
  ...
  begin
    ...
    tmpBlob.Blob:=Directions;
    //End;
    //If (Type=Type::Downhole) And ("Surface LSD"<>'' Then Begin
    //End;
    exit(tmpBlob.ReadAsText(CR, TextEncoding::Windows));
  end;
  local procedure GetLastLSDDirectionID(pLSDFormat: Code[20];
  pLSD: Code[50]): Code[20];
  var locLSDDirection: Record "ODT LSD Directions";
  begin
    //<NB03.87/>
    ...
    if locLSDDirection.FINDLAST then exit(locLSDDirection.ID)
    else
      exit('');
  end;
