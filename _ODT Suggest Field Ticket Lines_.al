report 50002 "ODT Suggest Field Ticket Lines"
{
    ...
    area(content)
    {
      group(Options)
      {
        ...
        field(JobTaskNoFilter;JobTaskNoFilter)
        {
          ...
          trigger OnLookup(var Text: Text): Boolean var JobTask: Record "Job Task";
          begin
  ...
  trigger OnPostReport()var NoSeriesMgt: Codeunit NoSeriesManagement;
  ...
  begin
    ...
    if TempTimeSheetLine.FindSet then begin
      ...
      repeat TimeSheetHeader.Get(TempTimeSheetLine."Time Sheet No.");
        ...
        if TimeSheetDetail.FindSet then repeat QtyToPost:=TimeSheetDetail.GetMaxQtyToPost;
            if QtyToPost <> 0 then begin
              ...
              if TempTimeSheetLine."Work Type Code" <> '' then begin
                ...
                if(WorkTypes."ODT Convert Res. to Item") and (workTypes."ODT Item No." <> '')then ConvertResToItem:=true
                else
  ...
  procedure SetJobJnlLine(NewJobJnlLine: Record "Job Journal Line")begin
    JobJnlLine:=NewJobJnlLine;
  end;
  [Scope('Personalization')]
  procedure SetFieldTicket(NewFT: record "ODT Field Ticket Job Mapping")begin
    FTHeader:=NewFT;
    JobNoFilter:=FTHeader."Job No.";
    JobTaskNoFilter:=FTHeader."Job Task No." end;
  [Scope('Personalization')]
  ...
  local procedure FillTimeSheetLineBuffer()var SkipLine: Boolean;
  begin
    ...
    if TimeSheetHeader.FindSet then repeat TimeSheetLine.SetRange("Time Sheet No.", TimeSheetHeader."No.");
        ...
        if TimeSheetLine.FindSet then repeat TempTimeSheetLine:=TimeSheetLine;
